...
bool BoardLogic::addMove(int move, char player) {
  if (this->boardSpaceAvailable(move) == true) {
    for (int i = 0; i < 6; i++) {
      if (board.at(move * 6 + i) == ' ') {
        board.at(move * 6 + i) = player;
        return true;
      }
    }
  } else
...
int BoardLogic::checkWin(int move, int height, char player, int count,
  ...
  if (board.at(move * 6 + height) != player)
    return 0;
...
bool BoardLogic::checkMoveMade(BoardLogic old) {
  for (int i = 0; i < 7; i++) {
    for (int j = 0; j < 6; j++) {
      if (this->board.at(i * 6 + j) != old.board.at(i * 6 + j)) {
        return true;
      }
    }
  }
